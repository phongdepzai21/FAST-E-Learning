...
const stateTarget = "const [status, setStatus] = useState<'idle' | 'sending' | 'success' | 'error'>('idle');";
...
if (code.includes('const FAQS =')) {
    ...
    code = code.replace('const Contact: React.FC = () => {', faqData + '\nconst Contact: React.FC = () => {');
    code = code.replace(stateTarget, stateReplacement);
    
    // Replace the end of main
    const lastSectionIndex = code.lastIndexOf('      </section>\n    </main>');
    if (lastSectionIndex !== -1) {
        code = code.substring(0, lastSectionIndex) + faqSection + code.substring(lastSectionIndex + '      </section>\n    </main>'.length);
        fs.writeFileSync('pages/Contact.tsx', code);
        console.log('FAQ section added successfully!');
    } else {
        console.log('Could not find end of main to inject FAQ.');
    }
}
