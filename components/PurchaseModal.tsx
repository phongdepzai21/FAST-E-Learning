    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
      <div className="bg-white rounded-3xl w-full max-w-md p-6 sm:p-8 shadow-2xl relative overflow-hidden">
        ...
        <div className={`absolute top-0 left-0 right-0 h-1.5 bg-blue-50 transition-opacity duration-300 ${isWorking ? 'opacity-100' : 'opacity-0'}`}>
          <div className="h-full bg-blue-500 w-full origin-left animate-[pulse_1.5s_ease-in-out_infinite]" style={{ animation: 'progress-indeterminate 1.5s infinite linear', transformOrigin: '0% 50%' }}>
            <style>{`
        ...
        {!otpSent ? (
          <div className="space-y-4">
            <p className="text-sm text-gray-600 text-center">
              Chúng tôi sẽ gửi một mã xác minh gồm 6 chữ số đến email{" "}
              <b className="text-gray-800">{userEmail}</b> của bạn để đảm bảo an toàn.
            </p>
            <button
              onClick={handleSendOtp}
              disabled={isSending}
              className="w-full py-3.5 px-4 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-2xl transition-all shadow-lg shadow-blue-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {isSending ? (
                <>
          ...
          <div className="space-y-5">
            <div>
              ...
              <input
                ...
                className="w-full text-center text-3xl font-bold tracking-[0.5em] py-3 border-2 border-gray-200 rounded-2xl focus:ring-4 focus:ring-blue-100 focus:border-blue-500 transition-all outline-none disabled:bg-gray-50 disabled:text-gray-400"
              />
            </div>
            <button
              onClick={handleVerify}
              disabled={otpCode.length !== 6 || isVerifying}
              className="w-full py-3.5 px-4 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-2xl transition-all shadow-lg shadow-green-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {isVerifying ? (
                <>
                  <svg className="animate-spin h-5 w-5 text-white" fill="none" viewBox="0 0 24 24">
